...
#define PROC_NAME "copy"
#define LOG_PROC_NAME "copy_log"
                        ...
                        struct sk_buff *skb, unsigned long *data)
{
	...
	if(ip->tot_len < COPY_MAX_BYTES ){
		fast_memcpy(table->data[cur_end], ip, ip->tot_len);
	}
	else{
		fast_memcpy(table->data[cur_end], ip, COPY_MAX_BYTES);
	}
	cur_end = cur_end + 1;
