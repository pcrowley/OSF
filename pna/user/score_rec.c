...
void print_sig(struct osf_sig *sig){
	...
	dst3 = ip_ptr[2];
	dst4 = ip_ptr[3];
	//printf("%c;%s:%s:%s:%u\n", sig->os_type, sig->os_class, sig->os_name, sig->os_flavor, sig->score);
	//return;
	printf("%u;%u;%u;%u;%u;%u;%u;%u;%c;%s;%s;%s;%u;%u;%u;%u;%u;%u;%u;%u;%u;%u;%u\n",
	src1, src2, src3, src4,
	dst1, dst2, dst3, dst4,
		...
		for(j=0 ; j<logs_per_ip ; j++){
			...
			if(j < cur_info.current_log){
				if(sig.print.db_entry < max_list){
					...
					sig.score = list[sig.print.db_entry].score;
