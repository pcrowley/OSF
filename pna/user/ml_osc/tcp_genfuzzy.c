...
void generate_hashes(unsigned int **rf_feature){
	...
	u32 res;
	u32 check[100000];
	u32 truth_rec[100000];
	u32 ignore_list[100000];
	for(i=0 ; i < num_packets ; i++){
	...
	for(i=0 ; i < 3 ; i++){
		for(j=0 ; j < 4 ; j++){
			for(k=0 ; k < num_packets ; k++){
				...
				if(j == 0){
					rf_feature[k][1] = 0;
					rf_feature[k][2] = rf_feature[k][2] & 5;
				}
				else if(j == 1){
					rf_feature[k][1] = 0;
					rf_feature[k][2] = rf_feature[k][2] | 2;
				}
				else if(j == 2){
					rf_feature[k][1] = 1;
					rf_feature[k][2] = rf_feature[k][2] & 5;
				...
				if(DEBUG_F){
					for(l=0 ; l < OSF_FEATURES ; l++){
						printf("%u ", rf_feature[k][l]);
					}
					printf("\n");
				}
				
				res = fnv_32a_buf((void *)rf_feature[k], OSF_FEATURES * sizeof(unsigned int), FNV1_32A_INIT);
				check[cur_check] = 0;
				check[cur_check++] = res;
			}
		}
	...
	for(i=0 ; i < num_packets ; i++){
		truth_rec[cur_truth++] = truths[i];
	}
	for(x=0 ; x < 2 ; x++){
	for(i=0 ; i < 3 ; i++){
		for(j=0 ; j < 4 ; j++){
			for(k=0 ; k < num_packets ; k++){
				truth_rec[cur_truth++] = truths[k];
			}
		}
	}
	}
	for(i=0 ; i < cur_check ; i++){
		ignore_list[i] = 0;
	}
	for(i=0 ; i < cur_check ; i++){
		for(j=i+1 ; j < cur_check ; j++){
			if(ignore_list[i]){
				continue;
			}
			if(check[i] == check[j]){
		...
		for(j=i+1 ; j < num_packets ; j++){
			...
			if(check[i] == check[j]){
				...
				for(k=0 ; k < OSF_FEATURES ; k++){
					printf("%u ", rf_feature[i][k]);
				}
				printf("\n");
				for(k=0 ; k < OSF_FEATURES ; k++){
					printf("%u ", rf_feature[j][k]);
				}
				printf("\n");
			}
		}
