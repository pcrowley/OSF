...
#define LOG_FILE_FORMAT  "%s/osf-%%Y%%m%%d%%H%%M%%S-%s.log"
#define MAX_STR          1024
...
void dump_table(void *table_base, char *out_file)
{
    ...
    struct osfmon_entry *flow_table;
    struct osf_log_hdr *log_header;
    struct osf_log_entry *log;
    char buf[BUF_SIZE];
    int buf_idx;
    ...
    lseek(fd, sizeof(struct osf_log_hdr), SEEK_SET);

    buf_idx = 0;
    nflows = 0;

	flow_table = (struct osfmon_entry *)table_base;
    ...
    for (flow_idx = 0 ; flow_idx < PNA_FLOW_ENTRIES; flow_idx++ ) {
		...
		log = (struct osf_log_entry *)&buf[buf_idx];
		buf_idx += sizeof(struct osf_log_entry);
		//88 Test
		//printf("%d\n", flow_idx);
		log->osf.local_ip = flow->local_ip;
		log->osf.tcp_sig.opt_hash = flow->tcp_sig.opt_hash;
		log->osf.tcp_sig.quirks = flow->tcp_sig.quirks;
		log->osf.tcp_sig.opt_eol_pad = flow->tcp_sig.opt_eol_pad;
		log->osf.tcp_sig.ip_opt_len = flow->tcp_sig.ip_opt_len;
		log->osf.tcp_sig.ip_ver = flow->tcp_sig.ip_ver;
		log->osf.tcp_sig.ttl = flow->tcp_sig.ttl;
		log->osf.tcp_sig.mss = flow->tcp_sig.mss;
		log->osf.tcp_sig.win = flow->tcp_sig.win;
		log->osf.tcp_sig.win_type = flow->tcp_sig.win_type;
		log->osf.tcp_sig.wscale = flow->tcp_sig.wscale;
		log->osf.tcp_sig.pay_class = flow->tcp_sig.pay_class;
		log->osf.tcp_sig.tot_hdr = flow->tcp_sig.tot_hdr;
		log->osf.tcp_sig.ts1 = flow->tcp_sig.ts1;
		log->osf.tcp_sig.recv_ms = flow->tcp_sig.recv_ms;
		memcpy(&(log->osf.tcp_mat), &(flow->tcp_mat), sizeof(struct osf_tcp_mat));		
		//log->osf.tcp_mat.type = flow->tcp_mat.type;
		//log->osf.tcp_mat.os_class = "XXXXX";
		//log->osf.tcp_mat.name = "XXXXXXXXXXXXXXXXXXXX";
		//log->osf.tcp_mat.flavor = "XXXXXXXXXXXXXXXXXXXX";
		//88 Test End
		/* copy the flow entry */
		/*
		log->osf.tcp_sig.opt_hash = flow->key.osf_record->tcp_sig.opt_hash;
		log->osf.tcp_sig.quirks = flow->key.osf_record->tcp_sig.quirks;
		log->osf.tcp_sig.opt_eol_pad = flow->key.osf_record->tcp_sig.opt_eol_pad;
		log->osf.tcp_sig.ip_opt_len = flow->key.osf_record->tcp_sig.ip_opt_len;
		log->osf.tcp_sig.ip_ver = flow->key.osf_record->tcp_sig.ip_ver;
		log->osf.tcp_sig.ttl = flow->key.osf_record->tcp_sig.ttl;
		log->osf.tcp_sig.mss = flow->key.osf_record->tcp_sig.mss;
		log->osf.tcp_sig.win = flow->key.osf_record->tcp_sig.win;
		log->osf.tcp_sig.win_type = flow->key.osf_record->tcp_sig.win_type;
		log->osf.tcp_sig.wscale = flow->key.osf_record->tcp_sig.wscale;
		log->osf.tcp_sig.pay_class = flow->key.osf_record->tcp_sig.pay_class;
		log->osf.tcp_sig.tot_hdr = flow->key.osf_record->tcp_sig.tot_hdr;
		log->osf.tcp_sig.ts1 = flow->key.osf_record->tcp_sig.ts1;
		log->osf.tcp_sig.recv_ms = flow->key.osf_record->tcp_sig.recv_ms;
		log->osf.tcp_mat.type = flow->key.osf_record->tcp_mat.type;
		*/
		//log->osf.tcp_mat.os_class = "XXXXX";
		//log->osf.tcp_mat.name = "XXXXXXXXXXXXXXXXXXXX";
		//log->osf.tcp_mat.flavor = "XXXXXXXXXXXXXXXXXXXX";
/*
For reference so I don't have to keep switching pages (Remove this later)
...
*/
		...
		if (buf_idx + sizeof(struct osf_log_entry) >= BUF_SIZE) {
			/* flush the buffer */
    ...
    lseek(fd, 0, SEEK_SET);
    log_header = (struct osf_log_hdr *)&buf[buf_idx];
    log_header->start_time = start_time;
    log_header->end_time = time(NULL);
    log_header->size = nflows * sizeof(struct osf_log_entry);
